...
import { resolveUserPath, toPosixRel, ensureUserDir } from "@/lib/storage";
...
async function sessionFromRequest(request: NextRequest) {
  ...
  if (basic?.[1]) {
    ...
    if (email && token) {
      ...
      if (row && row.user.email.toLowerCase() === email && row.expiresAt.getTime() > Date.now()) {
        return row.user;
      }
    }
  }
...
async function handleRead(
  ...
  if (!user) return new Response("Unauthorized", { status: 401, headers: { "WWW-Authenticate": 'Basic realm="HBS Cloud"' } });
  ...
  if (head.toString("utf8") === "HBS2") {
    ...
    return new Response(headOnly ? null : new Uint8Array(plain), {
      headers: { "Content-Length": String(plain.length), "Content-Type": "application/octet-stream" },
    });
  }
  const stat = fs.statSync(abs);
  if (headOnly) {
    return new Response(null, { headers: { "Content-Length": String(stat.size) } });
  }
...
export async function PUT(
  ...
  await prisma.backupFile.upsert({
    ...
    update: { size: BigInt(buf.length), name, searchName: name.toLowerCase() },
  });
...
export async function DELETE(
  ...
  await prisma.backupFile.deleteMany({
    where: { userId: user.id, OR: [{ path: rel }, { path: { startsWith: `${rel}/` } }] },
  });
...
export async function MKCOL(
  ...
  await prisma.backupFile.upsert({
    where: { userId_path: { userId: user.id, path: rel } },
    create: { userId: user.id, path: rel, name, parentPath, isDir: true, size: BigInt(0) },
    update: { isDir: true },
  });
  return new Response(null, { status: 201 });
}
...
export async function PROPFIND(
  ...
  const rows = await prisma.backupFile.findMany({
    where: depth === "0"
      ? { userId: user.id, path: parent || undefined, ...(parent ? {} : { parentPath: "" }) }
      : { userId: user.id, parentPath: parent },
    orderBy: [{ isDir: "desc" }, { name: "asc" }],
  });
  const self = parent
    ? await prisma.backupFile.findFirst({ where: { userId: user.id, path: parent } })
    : null;
  const entries = [
    ...(self ? [self] : [{ path: "", name: "", isDir: true, size: 0n, updatedAt: new Date() }]),
    ...(depth === "0" ? [] : rows),
  ];
...
export async function MOVE(
  ...
  const destPath = decodeURIComponent(destHeader.replace(/^https?:\/\/[^/]+/i, "").replace(/^\/dav\/?/, ""));
  const params = await context.params;
  const rel = toPosixRel((params.path || []).map(decodeURIComponent).join("/"));
  ...
  if (!fs.existsSync(fromAbs)) return new Response("Not found", { status: 404 });
  fs.mkdirSync(path.dirname(toAbs), { recursive: true });
  fs.renameSync(fromAbs, toAbs);
  const name = path.posix.basename(destRel);
  const parentPath = destRel.includes("/") ? destRel.slice(0, destRel.lastIndexOf("/")) : "";
