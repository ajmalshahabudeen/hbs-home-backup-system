...
import { requireSession, ok, badRequest, clientMeta, writeLog } from "@/lib/auth-guard";
  ...
  try {
    ...
    const device = await prisma.mobileDevice.upsert({
      ...
      create: {
        ...
        platform: platform ? String(platform).toLowerCase().trim() : "android",
      ...
      update: {
        deviceName: deviceName ? String(deviceName).trim() : undefined,
        platform: platform ? String(platform).toLowerCase().trim() : undefined,
        pushToken: pushToken ? String(pushToken).trim() : undefined,
        lastIp: ip,
        lastSeenAt: new Date(),
      },
    ...
    return badRequest(e instanceof Error ? e.message : "Failed to register device");
  }
}
