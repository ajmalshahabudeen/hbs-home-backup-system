...
export async function POST(request: NextRequest) {
  const { session, error } = await requireSession(request);
  if (error) return error;
  const body = (await request.json()) as { fileId?: string; version?: number };
  if (!body.fileId || !body.version) return badRequest("fileId and version required");
  const file = await prisma.backupFile.findFirst({
    where: { id: body.fileId, userId: session.user.id },
  });
  const ver = await prisma.fileVersion.findFirst({
    where: { fileId: body.fileId, version: Number(body.version), userId: session.user.id },
  });
