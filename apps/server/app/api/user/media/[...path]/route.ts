...
import { badRequest, requireSession } from "@/lib/auth-guard";
    ...
    try {
      ...
      if (isHeic && !wantOriginal) {
        try {
          ...
          const jpeg = await sharp(plain ?? /* turbopackIgnore: true */ abs, {
            failOn: "none",
          })
            .rotate()
      ...
      if (isRaw && !wantOriginal) {
        try {
          ...
          return new Response(
            "RAW preview unavailable. Download the original file.",
            {
              status: 415,
              headers: { "X-HBS-Raw": "1" },
            },
